 ...
 * `$DSH_HOME/qialike-titles.json`) at zero read cost, and the list lookup
...
const STATE_KEY = Symbol.for('qialike.session-titles.state')
 ...
 *  on delete but the pin set was not, so `qialike-pinned.json` kept the id of a
 ...
 *  `qialike resume`, and the SAME bit drives `/new`'s blank adoption
 ...
 *  session was used (`qialike-activity.json`, written on every switch/resume),
