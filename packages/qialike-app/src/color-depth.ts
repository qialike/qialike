...
/**
 * The explicit `QIALIKE_COLOR` override, or null when it is unset/unrecognised.
 *
...
export function colorOverride(env: NodeJS.ProcessEnv = process.env): ColorLevel | null {
  const override = (env.QIALIKE_COLOR ?? '').trim().toLowerCase()
 ...
 *
 * `QIALIKE_COLOR` wins when set (see {@link colorOverride}); otherwise the rules
