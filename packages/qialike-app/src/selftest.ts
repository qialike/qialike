 ...
 * checks against the running qialike and prints a ✓/✗ report into the
 ...
 *  - **external suite** (only when a qialike checkout with `tests/` and a
...
export function syncChecks(deps: SyncCheckDeps): CheckResult[] {
  ...
  //    `sol` is shared by solarized / solarized-light (one-dark and every
  //    other 3-char prefix are unique).
...
export function findRepo(): string | undefined {
  const candidates = [
    process.env.QIALIKE_SELFTEST_REPO,
    process.cwd(),
...
export function apply(ctx: Context): void {
  ...
  tui.commands.register({
    ...
    run: (arg) => {
      ...
      const externalBlocked = text === 'sync' || process.env.QIALIKE_SELFTEST_NO_EXTERNAL === '1'
      if (!externalBlocked || externalForced) {
        ...
        if (repo === undefined || !hasBun()) {
          appendLine(`selftest: external suite skipped (no qialike checkout or bun on PATH — SEA build runs the in-process checks only)`)
          return
        }
