 ...
 *  mapping) so qialike's headers read the same as deepseek-harness. Unknown
 ...
 *  not help" report (see qialike-development §2.5.51).
 *
...
function itemContent(item: TranscriptItem, expandReasoning: boolean, toolExpanded: boolean, compactionExpanded: boolean, hovered: boolean, usable: number, toolLive: boolean, reasoningLive: boolean): React.ReactNode {
  ...
  // surface before it enters the render tree (qialike-security.md). The store
...
const MemoTranscriptItemView = React.memo(function TranscriptItemView(props: {
  ...
  React.useEffect(() => {
    ...
    // the answer below the transcript viewport (qialike-development §2.5.51).
    if (ref.current) {
...
/** `QIALIKE_HERO_ART` override, read once at module load (auto by default). */
const HERO_ART_MODE = heroArtMode(process.env.QIALIKE_HERO_ART)
 ...
 *  `QIALIKE_HERO_ART_ENCODING` pins one for A/B. */
const HERO_ART_ENCODING = heroArtEncoding(process.platform, process.env.QIALIKE_HERO_ART_ENCODING)
...
/** Plan-B A/B switch (diagnosis only): `QIALIKE_LEGACY_EST=1` re-parses the
...
const legacyEstimate = /^(1|true|yes|on)$/i.test(process.env.QIALIKE_LEGACY_EST ?? '')
...
/** S0 diagnostic (`QIALIKE_DEBUG_EST=1`, `session/optimization-plan.md` §3):
...
const debugEst = /^(1|true|yes|on)$/i.test(process.env.QIALIKE_DEBUG_EST ?? '')
let estHits = 0
let estParses = 0
let estParseMs = 0
...
/** Attribution mode for the layout pass (`QIALIKE_DEBUG_LAYOUT=1`): times every
...
const debugLayout = /^(1|true|yes|on)$/i.test(process.env.QIALIKE_DEBUG_LAYOUT ?? '')
/** Threshold (ms) above which a layout pass is logged. 200 keeps the log quiet
 *  in normal runs; `QIALIKE_DEBUG_LAYOUT_MS=0` logs EVERY pass, which is what
...
const debugLayoutMs = (() => {
  const n = Number(process.env.QIALIKE_DEBUG_LAYOUT_MS ?? Number.NaN)
  return Number.isFinite(n) && n >= 0 ? n : 200
})()
/** Tail-geometry probe (`QIALIKE_DEBUG_TAIL=1`, diagnosis only): one line per
...
const debugTail = /^(1|true|yes|on)$/i.test(process.env.QIALIKE_DEBUG_TAIL ?? '')
 ...
 *  every 2 s. No behaviour change; off unless `QIALIKE_DEBUG_LAYOUT=1`. */
const perf = { frames: 0, rowsMs: 0, rowsMax: 0, itemsMs: 0, itemsMax: 0, at: 0, items: 0, heapMb: 0 }

/** WHOLE-FRAME gap probe (`QIALIKE_DEBUG_LAYOUT=1`): the two row builders are
...
function frameGapProbe(items: number, scroll: number, followTail: boolean): void {
  ...
  // beat (`[stall]`, QIALIKE_STALL_MS) for genuine main-thread wedges.
  ...
  // backpressure, a blocked writer) — use `[stall]`/`QIALIKE_STALL_MS` for
  // main-thread wedges.
...
/** Which LEFT-SLOT branch the status bar takes (`QIALIKE_DEBUG_STATUS=1`),
...
const debugStatus = /^(1|true|yes|on)$/i.test(process.env.QIALIKE_DEBUG_STATUS ?? '')
...
function conversationKey(k: RawKey, tui: TuiService): void {
  ...
  if (k.tab) {
    ...
    if (session !== undefined && store.readOnlySessionId === undefined) {
      ...
      // qialike-security.md). Cycling to danger-full-access raises the file
      ...
      <Box flexShrink={0} flexDirection="row" borderStyle="round" borderColor={theme.border} paddingX={1} height={STATUS_BAR_HEIGHT}>
        ...
        {/* The busy indicator (Working/Paused/Idle + icon) is REPLACED on the
            ...
            own reasoning sits with its helper:
              ...
              preparing    → two phases in one slot, each showing its OWN
                             information and clock: the assembly's
                             (`preparing the request… 4.4s`, shown only once a
                             tick proves the loop is free — `[stall]` otherwise),
                             then the provider wait's (`waiting for the model…
                             12.3s`, counting from the moment the payload left)
