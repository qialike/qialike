...
export function syncChecks(deps: SyncCheckDeps): CheckResult[] {
  ...
  // 1) colorscheme registry: 19 built-ins present (2 curated defaults — dark
  //    from opencode, light from DeepSeek Harness — + 3 optional skins + 14
  //    classic).
  const schemes = schemeRegistry()
  const required = ['dark', 'light', 'one-dark', 'one-light', 'dsh-dark', 'catppuccin', 'dracula', 'everforest', 'falcon', 'flexoki', 'gruvbox', 'jellybeans', 'kanagawa', 'monokai', 'nord', 'panda', 'rosepine', 'solarized', 'solarized-light']
  ...
  //    `one` is shared by one-dark/one-light and `sol` by solarized /
  //    solarized-light (dsh and every other 3-char prefix are unique).
