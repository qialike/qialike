...
每一帧都**先隐藏硬件光标再绘制**，帧尾后缀再把 caret 的形状与位置放回——因此光标不会被（可见地）拖着扫过重绘涉及的行。每一帧同时被终端的**同步输出**模式括起来（`ESC[?2026h` … `ESC[?2026l`）：一帧是整宽重绘（打开命令面板实测 120×30 下约 5.5 KB、240×30 下约 7.4 KB），而 pty 会把超过约 4095 字节行规程缓冲的写入分成多次交给终端，没有这个模式时半绘制的帧会短暂可见（输入卡片的边框/状态行从弹层里透出来）。不认这个模式的终端会忽略它；若某个终端处理不当，用 `DSH_TUI_NO_SYNC=1` 写普通帧。
